...
let pontos_verde = 0
let pontos_azul = 0
const messageElement = document.getElementById('pontos');

function atualizaAnimacao(){
  
    txBola += txBola_offset;
    //verificar se bola encostou na barra, ou seja se xbola >= ybarra E ybarra 1 <= ybola <= ybarra2
    
    if((txBola > 0.8 || txBola<-0.8) && ((tyBE-0.1 <= tyBola && tyBola<=tyBE+0.1)||(tyBD-0.1 <= tyBola && tyBola<=tyBD+0.1))){
        txBola_offset = -txBola_offset;
    }else if(txBola > 0.95 || txBola<-0.95){
        if(txBola > 0.95)//ponto verde
            pontos_verde += 1;
        if(txBola<-0.95)
            pontos_azul += 1;
        txBola = 0.0;
        ...
        txBola_offset = 0.000;
        tyBola_offset = 0.000;
        messageElement.textContent = `Placar: Verde ${pontos_verde} X ${pontos_azul} Azul`;
    }
